      ...
      .then((response) => {
        if (response?.success === true) {
          if (response?.user?.role === "mentee") {
            ...
            setTimeout(() => {
              navigate("/mentee"); // Redirect to the mentee dashboard
            }, 1000);
          } else {
      ...
      <div className="flex flex-1 justify-center items-center p-5">
        <div className="w-full max-w-md bg-white shadow-2xl rounded-lg p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="text-center">
              ...
              <p className="text-gray-500 mt-2 font-poppins">
                Enter your email and password below to access your mentee account.
              </p>
            </div>

            {/* Hidden Role Input - Explicitly set to mentee */}
            <input
              type="hidden"
              name="role"
              value="mentee"
              onChange={(e) => setInputValues((prev) => ({ ...prev, role: "mentee" }))}
            />
          ...
          <p className="text-center text-sm text-gray-600 font-poppins mt-4">
            ...
            <Link to="/registerMentee" className="font-medium text-green-500 hover:underline font-poppins">
              Sign up now
            </Link>
          </p>
        </div>
